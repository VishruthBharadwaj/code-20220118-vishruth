...
def read_contentss():
    list_of_row = []
    with open('tabledata.csv') as file:
...
def finaldata_with_bmii(alldata):
    line_count = 0
    notValidData = []
    validData = []
    for row in alldata:
...
def write_csv(data):
    with open('output_for_BMI.csv', 'w',newline='') as file:
...
if __name__ == '__main__':
    contents = read_contentss()
    # print(result)
    finaldata = finaldata_with_bmii(contents)
